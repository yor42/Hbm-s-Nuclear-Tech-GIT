...
public class GUIMicrowave extends GuiInfoContainer {
	...
	public static ResourceLocation texture = new ResourceLocation(Tags.MOD_ID + ":textures/gui/processing/gui_microwave.png");
	...
	protected void mouseClicked(int x, int y, int i) throws IOException {
	    ...
	    if(guiLeft + 43 <= x && guiLeft + 43 + 18 > x && guiTop + 25 < y && guiTop + 25 + 18 >= y) {
	    	PacketDispatcher.wrapper.sendToServer(new AuxButtonPacket(microwave.getPos(), 0, 0));
	    }
	    if(guiLeft + 43 <= x && guiLeft + 43 + 18 > x && guiTop + 43 < y && guiTop + 43 + 18 >= y) {
	    	PacketDispatcher.wrapper.sendToServer(new AuxButtonPacket(microwave.getPos(), 1, 0));
	    }
    }
