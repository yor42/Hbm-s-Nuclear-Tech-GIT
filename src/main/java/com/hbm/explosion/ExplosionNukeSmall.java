...
public class ExplosionNukeSmall {
	...
	public static void explode(World world, double posX, double posY, double posZ, int size) {
		...
		if(size == safe) {
			...
			world.spawnEntity(EntityNukeExplosionMK4.statFac(world, BombConfig.fatmanRadius, posX, posY, posZ).mute());
		}
		...
		for(int i = -2; i <= 2; i++)
			for(int j = -2; j <= 2; j++)
				if(i + j < 4)
					RadiationSavedData.incrementRad(world, new BlockPos((int)posX + i * 16, posY, (int)posZ + j * 16), 50 / (Math.abs(i) + Math.abs(j) + 1) * radMod, 1000);
	}
}