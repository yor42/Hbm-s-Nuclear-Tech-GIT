...
public class TurretMaxwell extends BlockDummyable {
	...
	public boolean onBlockActivated(World world, BlockPos bpos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ){
		if(world.isRemote) {
			...
			FMLNetworkHandler.openGui(player, MainRegistry.instance, ModBlocks.guiID_maxwell, world, pos[0], pos[1], pos[2]);
			return true;
		} else {
			return false;
		}
